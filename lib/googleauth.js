...
function loadCache(oauth2Client, callback) {
	// load credentials from disk
	if (fs.existsSync(cache_location)) {
		var credentials = JSON.parse(fs.readFileSync(cache_location))
		...
		// see if it needs to refresh
		var age = Date.now() - fs.statSync(cache_location).mtime
		if (age > credentials.expires_in * 1e3) {
...
function make_userinfo_oauth(callback) {
	...
	// load credentials from disk
	loadCache(oauth2Client, function() {
